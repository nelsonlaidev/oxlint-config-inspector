...
if (oxlintRules.length === 0) {
  throw new Error('`oxlint --rules` returned no rules.')
}
...
const failedRules: RuleDocFetchResult[] = []
...
if (failedRules.length > 0) {
  process.stdout.write(`Failed to fetch ${failedRules.length} rule docs:\n`)

  for (const failed of failedRules) {
    process.stdout.write(`- ${getBuiltinRuleId(failed.rule)}: ${failed.failure ?? 'unknown error'}\n`)
  }

  throw new Error('Aborted: no output written.')
}

await mkdir(path.dirname(OUTPUT_PATH), { recursive: true })
await writeFile(OUTPUT_PATH, createGeneratedFile(metadataByRuleId, oxlintVersion))
...
async function fetchWithRetry(url: string, attempts = 3, attempt = 1): Promise<Response> {
  let failure: unknown
  ...
  try {
    ...
    failure = new Error(`HTTP ${response.status} ${response.statusText}`)
  } catch (error) {
    failure = error
  }
...
async function getOxlintVersion() {
  try {
    const version = await x('oxlint', ['--version'])
    const output = version.stdout.trim()
    const versionNumber = output.includes(':') ? output.slice(output.indexOf(':') + 1).trim() : output

    return versionNumber.length > 0 ? versionNumber : null
  } catch {
    return null
  }
}
...
function createGeneratedFile(metadataByRuleId: Record<string, BuiltinRuleDocMetadata>, oxlintVersion: string | null) {
  const versionHeader = oxlintVersion ? `\n// Oxlint version: ${oxlintVersion}` : ''

  return `\
// oxlint-disable eslint/no-template-curly-in-string
// This file is generated by scripts/generate-builtin-rule-docs.ts.
// Do not edit this file manually.${versionHeader}
